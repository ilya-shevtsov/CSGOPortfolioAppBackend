...
import features.caseoverview.domain.entities.CaseMapper
...
class CaseRepositoryImpl @Inject constructor(
    ...
    override fun getCaseData(): List<Case> {
        return transaction {
            CaseTable.selectAll().map { CaseDboMapper.map(it) }
        }.map { case -> CaseMapper.map(case) }
    }
    ...
    override fun insertInitialDataCaseOverview() {
        ...
        transaction {
            val storedCaseList = getStoredCaseList()
